...
- Create: `services/core/eshopeo/db/migrations/versions/0002_tenant_pack_id.py`
- Modify: `services/core/eshopeo/api/routers/tenants.py`
- Test: `services/core/tests/test_tenant_pack.py` (first 2 tests only — pack routing tests come in Task 2)

**Context:**
- `Tenant` model is in `services/core/eshopeo/db/models.py`
- Existing migration pattern: see `services/core/eshopeo/db/migrations/versions/0001_initial.py`
...
In `services/core/eshopeo/db/models.py`, inside `class Tenant`, add after `created_at`:
```python
...
In `services/core/eshopeo/api/routers/tenants.py`, update `ProvisionRequest`:
```python
...
git add services/core/eshopeo/db/models.py \
        ...
        services/core/eshopeo/api/routers/tenants.py \
        services/core/tests/test_tenant_pack.py
...
**Files:**
- Modify: `services/core/eshopeo/packs/registry.py`
...
In `services/core/eshopeo/packs/registry.py`, add after `default_pack()`:
```python
from eshopeo.db.models import Tenant
...
In `services/core/eshopeo/api/routers/widget.py`:
- Change `from eshopeo.packs.registry import default_pack` to `from eshopeo.packs.registry import get_pack_for_tenant`
...
In `services/core/eshopeo/api/routers/sync.py`:
- Change `from eshopeo.packs.registry import default_pack` to `from eshopeo.packs.registry import get_pack_for_tenant`
...
```
git add services/core/eshopeo/packs/registry.py \
        services/core/eshopeo/api/routers/widget.py \
        services/core/eshopeo/api/routers/sync.py \
        services/core/tests/test_tenant_pack.py
...
- Auth: `X-eShopeo-Provision-Key` header (same as provisioning) — check against `settings.provision_key.get_secret_value()`
...
def client(tenant):
    ...
    import eshopeo.db.crud.tenants as tenants_crud
...
Append to `services/core/eshopeo/db/crud/tenants.py`:
```python
...
Add these imports at the top of `services/core/eshopeo/api/routers/tenants.py`:
```python
...
```
git add services/core/eshopeo/db/crud/tenants.py \
        services/core/eshopeo/api/routers/tenants.py \
        services/core/tests/test_tenant_management.py
...
**Files:**
- Create: `services/core/eshopeo/db/crud/jobs.py`
- Create: `services/core/eshopeo/api/routers/jobs.py`
...
- Auth: `X-eShopeo-Tenant-Key` (same as sync/search endpoints) via `get_tenant` dep
...
def client(tenant, job):
    ...
    import eshopeo.db.crud.jobs as jobs_crud
...
- [ ] **Step 3: Create `services/core/eshopeo/db/crud/jobs.py`**
...
- [ ] **Step 4: Create `services/core/eshopeo/api/routers/jobs.py`**
...
from eshopeo.api.deps import get_db, get_tenant
from eshopeo.db.crud import jobs as jobs_crud
from eshopeo.db.models import Tenant
...
In `services/core/eshopeo/api/app.py`, add after the analytics router block:
```python
...
```
git add services/core/eshopeo/db/crud/jobs.py \
...
- The JS: reads `key` from URL params of the current `<script>` tag's `src`; calls `/v1/widget/session` with `X-eShopeo-Tenant-Key: <key>` to get JWT; stores token in localStorage; injects a floating button and chat pane; sends messages to `/v1/widget/chat`
...
from eshopeo.config import get_settings as _get_settings
```
...
(function () {
  ...
  var key = (new URLSearchParams(script && script.src ? new URL(script.src).search : '')).get('key')
    || (script && script.getAttribute('data-eshopeo-key'));

  if (!key) { console.warn('[eShopeo] No key provided'); return; }

  var LS_TOKEN = 'eshopeo_token_' + key;
  var LS_EXP   = 'eshopeo_token_exp_' + key;
  ...
  function getToken() {
    ...
    return fetch(base + '/v1/widget/session', {
      method: 'POST',
      headers: { 'X-eShopeo-Tenant-Key': key, 'Content-Type': 'application/json' },
      body: '{}'
    })
  ...
  style.textContent = [
    '#eshopeo-btn{position:fixed;bottom:24px;right:24px;width:52px;height:52px;border-radius:50%;',
    'background:#6c47ff;border:none;cursor:pointer;color:#fff;font-size:22px;box-shadow:0 4px 12px rgba(0,0,0,.25);}',
    '#eshopeo-panel{display:none;position:fixed;bottom:88px;right:24px;width:340px;max-height:480px;',
    'background:#fff;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.15);',
    'display:none;flex-direction:column;overflow:hidden;}',
    '#eshopeo-messages{flex:1;overflow-y:auto;padding:16px;font-family:sans-serif;font-size:14px;}',
    '#eshopeo-msg-row{display:flex;padding:8px;border-top:1px solid #f0f0f0;}',
    '#eshopeo-input{flex:1;border:1px solid #ddd;border-radius:6px;padding:6px 10px;font-size:14px;outline:none;}',
    '#eshopeo-send{margin-left:8px;background:#6c47ff;color:#fff;border:none;border-radius:6px;',
  ...
  panel.id = 'eshopeo-panel';
  panel.innerHTML = [
    '<div id="eshopeo-messages"></div>',
    '<div id="eshopeo-msg-row">',
    '<input id="eshopeo-input" placeholder="Ask anything..." />',
    '<button id="eshopeo-send">Send</button>',
    '</div>'
  ].join('');
  ...
  function addMsg(text, role) {
    var d = document.getElementById('eshopeo-messages');
    var p = document.createElement('p');
    p.style.margin = '4px 0';
    p.style.color = role === 'user' ? '#333' : '#6c47ff';
    p.textContent = (role === 'user' ? 'You: ' : 'eShopeo: ') + text;
    d.appendChild(p);
    d.scrollTop = d.scrollHeight;
  }

  function send() {
    var input = document.getElementById('eshopeo-input');
    ...
    .then(function(r){ return r.json(); })
    .then(function(d){ addMsg(d.response || d.detail || 'Error', 'eshopeo'); })
    .catch(function(){ addMsg('Could not reach eShopeo. Please try again.', 'eshopeo'); });
  }

  document.getElementById('eshopeo-send').addEventListener('click', send);
  document.getElementById('eshopeo-input').addEventListener('keydown', function(e){
    if (e.key === 'Enter') send();
  });
})();
""".strip()
...
async def widget_demo_html() -> Response:
    ...
    html = """<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>eShopeo Widget Demo</title></head>
<body style="font-family:sans-serif;padding:40px;background:#f9f9f9;">
<h1>eShopeo Widget Demo</h1>
