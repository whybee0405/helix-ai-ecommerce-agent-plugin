...
Widget JS makes `fetch()` calls from the merchant's storefront domain to the eShopeo API. Without CORS, every browser blocks those requests.

### Settings change

Add to `eshopeo/config.py` → `Settings`:
```python
...
In `eshopeo/api/app.py`, add **before** RateLimitMiddleware (outermost layer processes first):
```python
...
app.add_middleware(
    ...
    allow_headers=["Authorization", "Content-Type", "X-eShopeo-Tenant-Key", "X-eShopeo-Provision-Key"],
...
### Implementation: `eshopeo/api/middleware/request_id.py`
...
Closes the data loop. `CanonicalOrder` is already defined in `eshopeo/connectors/models.py`. The `Order` model exists in `eshopeo/db/models.py`.

### New: `eshopeo/db/crud/orders.py`
...
### Endpoint: extend `eshopeo/api/routers/sync.py`
...
### Middleware: `eshopeo/api/middleware/quota.py`
...
- Headers: `X-eShopeo-Tenant-Id`, `X-WC-Webhook-Signature`
...
New function in `eshopeo/connectors/woocommerce.py` (if it exists) or inline in the router:
...
**New files:**
- `services/core/eshopeo/api/middleware/request_id.py`
- `services/core/eshopeo/api/middleware/quota.py`
- `services/core/eshopeo/db/crud/orders.py`

**Modified files:**
- `services/core/eshopeo/config.py` — add `cors_allowed_origins`, `default_monthly_query_limit`
- `services/core/eshopeo/api/app.py` — register CORSMiddleware, RequestIdMiddleware, QuotaMiddleware
- `services/core/eshopeo/api/routers/sync.py` — add `POST /v1/sync/orders`
- `services/core/eshopeo/api/routers/webhooks.py` — add `POST /v1/webhooks/woocommerce/orders`
