...
class Eshopeo_Backup {

    const CRON_HOOK    = 'eshopeo_db_backup';
    const BACKUP_DIR   = 'eshopeo-backups';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_eshopeo_run_backup',      [ __CLASS__, 'ajax_run' ] );
        add_action( 'wp_ajax_eshopeo_list_backups',    [ __CLASS__, 'ajax_list' ] );
        add_action( 'wp_ajax_eshopeo_download_backup', [ __CLASS__, 'ajax_download' ] );
        add_action( 'wp_ajax_eshopeo_delete_backup',   [ __CLASS__, 'ajax_delete' ] );
    }
    ...
    public static function maybe_schedule(): void {
        $schedule = get_option( 'eshopeo_backup_schedule', 'daily' );
        if ( $schedule === 'off' ) {
            return;
        }
    ...
    public static function run_backup(): array {
        ...
        update_option( 'eshopeo_backup_last_run', current_time( 'mysql' ), false );
    ...
    private static function prune_old_backups(): void {
        $keep     = max( 1, (int) get_option( 'eshopeo_backup_keep', self::KEEP_BACKUPS ) );
        $dir      = self::backup_dir();
