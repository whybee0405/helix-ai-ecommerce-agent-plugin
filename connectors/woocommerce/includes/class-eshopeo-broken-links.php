...
class Eshopeo_Broken_Links {

    const CACHE_OPTION  = 'eshopeo_broken_links_cache';
    const CACHE_TTL     = DAY_IN_SECONDS;

    /* ── Init ──────────────────────────────────────────────────────────── */

    public static function init(): void {
        add_action( 'wp_ajax_eshopeo_scan_broken_links',         [ __CLASS__, 'ajax_scan' ] );
        add_action( 'wp_ajax_eshopeo_broken_links_fix',          [ __CLASS__, 'ajax_fix_redirect' ] );
        add_action( 'wp_ajax_eshopeo_broken_links_fix_redirect', [ __CLASS__, 'ajax_fix_redirect' ] );
    }
    ...
    public static function ajax_scan(): void {
        ...
        foreach ( $batch_links as $link ) {
            $response = wp_remote_head( $link['url'], [
                'timeout'     => 8,
                'redirection' => 5,
                'user-agent'  => 'eShopeo-Broken-Link-Checker/1.0',
            ] );
    ...
    public static function ajax_fix_redirect(): void {
        ...
        Eshopeo_Snapshot::save( 'fix_broken_link_' . $post_id, [
            'post_id'      => $post_id,
            'old_url'      => $old_url,
            'new_url'      => $new_url,
