...
/**
 * Helix_Snippets — lightweight code snippets manager.
 *
 * Stores custom PHP snippets in {prefix}helix_snippets, loads enabled ones
 * on init priority 99, and exposes AJAX CRUD.
 */
class Helix_Snippets {

    const TABLE = 'helix_snippets';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_helix_snippets_list',   [ __CLASS__, 'ajax_list' ] );
        add_action( 'wp_ajax_helix_snippets_save',   [ __CLASS__, 'ajax_save' ] );
        add_action( 'wp_ajax_helix_snippets_delete', [ __CLASS__, 'ajax_delete' ] );
        add_action( 'wp_ajax_helix_snippets_toggle', [ __CLASS__, 'ajax_toggle' ] );
    }
        ...
        foreach ( $rows as $row ) {
            ...
            add_action( $hook, static function () use ( $snippet_id, $snippet_code ) {
                try {
                    ...
                    global $wpdb;
                    $wpdb->update(
                        $wpdb->prefix . Helix_Snippets::TABLE,
                        [ 'enabled' => 0 ],
                        [ 'id' => $snippet_id ]
                    );
