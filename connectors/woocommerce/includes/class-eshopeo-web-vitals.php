...
/**
 * Eshopeo_Web_Vitals — PageSpeed Insights integration for Core Web Vitals.
 */
class Eshopeo_Web_Vitals {

    const CRON_HOOK     = 'eshopeo_web_vitals_check';
    const CACHE_OPTION  = 'eshopeo_web_vitals_cache';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_eshopeo_web_vitals_scan', [ __CLASS__, 'ajax_scan' ] );
    }

    public static function maybe_schedule(): void {
        if ( ! get_option( 'eshopeo_pagespeed_api_key', '' ) ) {
            return;
        }
    ...
    private static function fetch_metrics( string $strategy = 'mobile' ): array|false {
        $api_key = get_option( 'eshopeo_pagespeed_api_key', '' );
        if ( ! $api_key ) {
            return false;
        }
    ...
    public static function ajax_scan(): void {
        ...
        $api_key = get_option( 'eshopeo_pagespeed_api_key', '' );
        if ( ! $api_key ) {
            wp_send_json_error( 'PageSpeed API key not configured. Add it in eShopeo Settings.', 400 );
        }
