...
class Helix_Uptime {
    ...
    const CRON_HOOK     = 'helix_uptime_check';
    const LOG_OPTION    = 'helix_uptime_log';
    const MAX_ENTRIES   = 288; // 24h × 12 per hour.
    const ALERT_OPTION  = 'helix_uptime_alert_email';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_helix_uptime_data', [ __CLASS__, 'ajax_data' ] );
    }
    ...
    public static function add_cron_interval( array $schedules ): array {
        if ( ! isset( $schedules['every_5_minutes'] ) ) {
            $schedules['every_5_minutes'] = [
                ...
                'display'  => __( 'Every 5 Minutes', 'helix-connector' ),
            ];
        }
        return $schedules;
    }
    ...
    private static function maybe_alert( array $log ): void {
        ...
        $last_alert = (int) get_option( 'helix_uptime_last_alert', 0 );
        if ( time() - $last_alert < HOUR_IN_SECONDS ) {
            return;
        }

        update_option( 'helix_uptime_last_alert', time(), false );

        $subject = sprintf( '[Helix] Site Down Alert — %s', get_bloginfo( 'name' ) );
        $message = sprintf(
