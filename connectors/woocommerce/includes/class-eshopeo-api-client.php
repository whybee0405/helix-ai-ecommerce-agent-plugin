...
class Eshopeo_API_Client {
    ...
    public function patch_tenant( string $tenant_id, array $patch ): array|WP_Error {
        ...
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) {
            return new WP_Error( 'eshopeo_patch_tenant_failed', "eShopeo API returned HTTP {$code}" );
        }
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function get( string $path ): array|WP_Error {
        $response = wp_remote_get( $this->api_url . $path, [
            'headers' => [ 'X-eShopeo-Tenant-Key' => $this->tenant_key ],
            'timeout' => 15,
        ] );
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_api_error', "HTTP {$code}", $response );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function bootstrap_admin_secret(): array|WP_Error {
        ...
        if ( ! $tenant_id || ! $provision_key ) {
            return new WP_Error( 'eshopeo_no_tenant', 'Tenant not provisioned' );
        }
        $response = wp_remote_post(
            $this->api_url . '/v1/tenants/' . $tenant_id . '/admin-secret',
            [
                'headers' => [ 'X-eShopeo-Provision-Key' => $provision_key ],
                'timeout' => 15,
            ]
        );
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code !== 200 ) return new WP_Error( 'eshopeo_bootstrap_failed', "HTTP {$code}" );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function get_branding(): array|WP_Error {
        ...
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_branding_get_failed', "HTTP {$code}" );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function update_branding( array $patch ): array|WP_Error {
        ...
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_branding_save_failed', "HTTP {$code}: " . wp_remote_retrieve_body( $response ) );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function apply_branding_preset( string $preset_id ): array|WP_Error {
        ...
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_preset_apply_failed', "HTTP {$code}" );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function list_presets(): array|WP_Error {
        ...
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_presets_failed', "HTTP {$code}" );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
    ...
    public function get_leads( int $page = 1, int $limit = 50 ): array|WP_Error {
        ...
        if ( is_wp_error( $response ) ) return $response;
        $code = wp_remote_retrieve_response_code( $response );
        if ( $code >= 400 ) return new WP_Error( 'eshopeo_leads_failed', "HTTP {$code}" );
        return json_decode( wp_remote_retrieve_body( $response ), true ) ?? [];
    }
