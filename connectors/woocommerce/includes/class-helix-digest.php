...
/**
 * Helix_Digest — weekly AI-generated site digest email.
 */
class Helix_Digest {

    const CRON_HOOK = 'helix_weekly_digest';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_helix_digest_send_test', [ __CLASS__, 'ajax_send_test' ] );
    }
    ...
    public static function maybe_schedule(): void {
        if ( ! get_option( 'helix_digest_enabled', 0 ) ) {
            return;
        }
    ...
    private static function collect_stats(): array {
        ...
        $broken_links = 0;
        $cached = get_option( 'helix_broken_links_cache' );
        if ( $cached ) {
        ...
        $uptime_pct = 100.0;
        $uptime_log = json_decode( get_option( 'helix_uptime_log', '[]' ), true );
    ...
    public static function send_digest( string $to = '' ): bool {
        ...
        $email = $to ?: get_option( 'helix_digest_email', get_option( 'admin_email', '' ) );
        if ( ! $email ) {
            return false;
        }
