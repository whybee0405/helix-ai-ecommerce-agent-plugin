...
class Helix_Quick_Actions {
    ...
    public static function init(): void {
        ...
        foreach ( $ids as $id ) {
            add_action( 'wp_ajax_helix_action_' . $id, [ __CLASS__, 'ajax_dispatch' ] );
        }
        add_action( 'wp_ajax_helix_wp_agent_actions', [ __CLASS__, 'ajax_list_actions' ] );
    }
    ...
    public static function ajax_dispatch(): void {
        ...
        $action_id = str_replace( 'helix_action_', '', $hook );
    ...
    public static function action_delete_revisions( bool $dry_run ): array {
        ...
        $snap_id = null;
        if ( $count > 0 ) {
            $snap_id = Helix_Snapshot::save( 'delete_revisions', [
    ...
    public static function action_delete_orphaned_meta( bool $dry_run ): array {
        ...
        $snap_id = null;
        if ( ! empty( $orphans ) ) {
            $snap_id = Helix_Snapshot::save( 'delete_orphaned_meta', [
    ...
    public static function action_fix_autoload( bool $dry_run ): array {
        ...
        $snap_id = Helix_Snapshot::save( 'fix_autoload', [
            'action' => 'fix_autoload',
            'rows'   => $snap_data,
        ] );
    ...
    public static function action_set_heartbeat( bool $dry_run ): array {
        $current = get_option( 'helix_heartbeat_interval', 'default' );
        ...
        $snap_id = Helix_Snapshot::save( 'set_heartbeat', [
            'action'   => 'set_heartbeat',
            'old_value' => $current,
        ] );

        update_option( 'helix_heartbeat_interval', 60, false );
    ...
    public static function action_disable_heartbeat_frontend( bool $dry_run ): array {
        $current = get_option( 'helix_disable_heartbeat_frontend', 0 );
        ...
        $snap_id = Helix_Snapshot::save( 'disable_heartbeat_frontend', [
            'action'    => 'disable_heartbeat_frontend',
            'old_value' => $current,
        ] );

        update_option( 'helix_disable_heartbeat_frontend', 1, false );
