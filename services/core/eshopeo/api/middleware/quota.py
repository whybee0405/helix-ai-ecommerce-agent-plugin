...
from eshopeo.billing.plans import get_query_limit
...
class QuotaMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if count > limit:
                ...
                return JSONResponse(
                    status_code=429,
                    content={"detail": "Monthly query limit exceeded. Upgrade your plan at eshopeo.cloudia.co.za"},
    ...
    def _status_to_response(status_str: str) -> JSONResponse | None:
        if status_str == "trial_expired":
            return JSONResponse(
                status_code=402,
                content={"detail": "Your free trial has ended. Subscribe at eshopeo.cloudia.co.za to restore access."},
                headers={"X-eShopeo-Status": "trial_expired"},
            )
        if status_str == "paused":
            return JSONResponse(
                status_code=402,
                content={"detail": "Your subscription is paused due to a failed payment. Update your payment method at eshopeo.cloudia.co.za"},
                headers={"X-eShopeo-Status": "paused"},
            )
        if status_str == "cancelled":
            return JSONResponse(
                status_code=402,
                content={"detail": "Your subscription has been cancelled. Resubscribe at eshopeo.cloudia.co.za"},
                headers={"X-eShopeo-Status": "cancelled"},
            )
        return None
