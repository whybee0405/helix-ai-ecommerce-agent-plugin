...
from eshopeo.domain.routine import build_routine
...
from eshopeo.db.crud.widget_events import create_widget_event
...
(function () {
  ...
  if (!KEY) { console.warn('[eShopeo] Missing key param'); return; }
  ...
  panel.innerHTML = [
    ...
    '<div class="hx-ht"><strong id="hx-brand-name">eShopeo</strong><span id="hx-brand-tagline">Your AI shopping assistant</span></div>',
  ...
  function applyEmbedBranding(B) {
    ...
    if (B.greeting && (gh || gp)) {
      var sub = window.eShopeoBranding.substitute(B.greeting, B);
      var parts = sub.split(/\n+/);
  ...
  function _cartNonce() {
    return (window.eshopeoCartNonce) || null;
  }

  function _cartItemKey(productId) {
    var items = window.eshopeoCartItems || [];
  ...
  function _execCartAction(action, productId, quantity) {
    ...
    fetch(url, { method: method, headers: headers, credentials: 'include', body: body })
      .then(function (r) {
        ...
        if (newNonce) { window.eshopeoCartNonce = newNonce; }
        if (r.ok) {
          return r.json().then(function (d) {
            /* Refresh cart items list from response */
            if (d && d.items) window.eshopeoCartItems = d.items.map(function (it) {
  ...
  function send(query) {
    ...
    getToken()
      ...
      .catch(function () {
        ...
        inp.disabled = false;
        if (streamRow) { streamRow.remove(); streamRow = null; }
        appendMsg('Could not reach ' + window.eShopeoBranding.substitute('{{brand_short_name}}') + '. Please try again.', 'bot', []);
      });
  }
  ...
  function _initInlineChats() {
    document.querySelectorAll('.eshopeo-inline-chat').forEach(function (el) {
...
async def _run_chat_pipeline(
    ...
    from eshopeo.branding import get_effective_branding
    branding = await get_effective_branding(tenant)

    # ── Budget / rate-limit guard ─────────────────────────────────────────
    import redis.asyncio as _aioredis
    from eshopeo.llm.budget import BudgetMode, acquire_token, check_budget
    ...
    from eshopeo.llm.history import HistoryCompressor
...
async def widget_chat_stream(
    ...
    import redis.asyncio as _aioredis
    from eshopeo.llm.budget import acquire_token, check_budget
    _redis = _aioredis.from_url(str(settings.redis_url), decode_responses=True)
    try:
        if not await acquire_token(_redis, tenant.id):
    ...
    from eshopeo.domain.consultant import (
...
(function () {
  ...
  var _lsTimer   = null;
  var _lsCtrl    = null;
  var AJAX_URL   = (window.eshopeoConfig && window.eshopeoConfig.ajaxUrl) || null;
  var _sbAnim    = !(window.eshopeoConfig && window.eshopeoConfig.sbAnim    === false);
  var _flyCart   = !(window.eshopeoConfig && window.eshopeoConfig.flyCart   === false);
  var _cardModal = !(window.eshopeoConfig && window.eshopeoConfig.cardModal === false);
  var _ctaType   = 'cart'; /* overridden to 'enquire' for automotive pack */
  ...
  root.innerHTML = [
    '<p id="hx-sb-headline">Ask eShopeo anything about our products</p>',
    '<div id="hx-sb">',
      '<div id="hx-sb-outer">',
  ...
  function applyBranding(B) {
    if (!B) return;
    if (headline) headline.textContent = window.eShopeoBranding.substitute(B.headline_text || '', B);
  ...
  function doLiveSearch() {
    ...
    openPanel();
    fetch(AJAX_URL + '?action=eshopeo_live_search&q=' + encodeURIComponent(q), { signal: _lsCtrl.signal })
      .then(function (r) { return r.json(); })
  ...
  /* ── "Open in chat" — forwards query to the main eShopeo chat panel ─────── */
...
(function () {
  if (window.eShopeoBranding) return;
