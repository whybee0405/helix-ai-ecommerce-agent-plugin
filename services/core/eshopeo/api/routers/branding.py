...
from eshopeo.db.crud.tenants import get_tenant_by_id, get_tenant_by_public_key, update_tenant
...
async def _auth_admin(
    request: Request,
    x_eshopeo_tenant_id: Annotated[str | None, Header()] = None,
    x_eshopeo_timestamp: Annotated[str | None, Header()] = None,
    x_eshopeo_signature: Annotated[str | None, Header()] = None,
    db: AsyncSession = Depends(get_db),
) -> Tenant:
    if not (x_eshopeo_tenant_id and x_eshopeo_timestamp and x_eshopeo_signature):
        raise HTTPException(
    ...
    if not _verify_admin_hmac(body, x_eshopeo_timestamp, x_eshopeo_signature, secret):
        raise HTTPException(
...
async def admin_create_faq(
    ...
    try:
        from eshopeo.workers.tasks.faq_embedding import embed_faq
