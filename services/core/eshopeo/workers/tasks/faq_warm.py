...
@celery_app.task(name="eshopeo.workers.tasks.faq_warm.warm_tenant_faq", queue="embedding")
def warm_tenant_faq(tenant_id_str: str) -> dict:
    return asyncio.run(_warm_async(UUID(tenant_id_str)))


@celery_app.task(name="eshopeo.workers.tasks.faq_warm.warm_all_tenants", queue="embedding")
...
async def _warm_all_async() -> dict:
    from eshopeo.db.crud.tenants import list_tenants
    from eshopeo.db.engine import async_session_factory
