...
celery_app = Celery(
    ...
    backend=_backend,
    include=[
        "eshopeo.workers.tasks.embedding",
        "eshopeo.workers.tasks.faq_warm",
    ],
)
celery_app.conf.update(
    ...
    task_routes={
        ...
        "eshopeo.workers.tasks.faq_warm.*": {"queue": "embedding"},
    },
    beat_schedule={
        "warm-faq-cache-weekly": {
            "task": "eshopeo.workers.tasks.faq_warm.warm_all_tenants",
            "schedule": 7 * 24 * 60 * 60,  # weekly
        },
        "trial-reminders-daily": {
            "task": "eshopeo.workers.tasks.trial_reminders.send_trial_reminders",
