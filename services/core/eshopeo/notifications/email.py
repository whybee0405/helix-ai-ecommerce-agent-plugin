...
_FROM = "eShopeo <hello@eshopeo.cloudia.co.za>"
...
def _base(content: str) -> str:
    ...
    <div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto;color:#111;">
      <div style="background:#18181b;padding:24px 32px;border-radius:12px 12px 0 0;">
        <span style="color:#a78bfa;font-weight:700;font-size:18px;">⬡ eShopeo</span>
      </div>
      ...
      <p style="font-size:11px;color:#9ca3af;text-align:center;margin-top:16px;">
        eShopeo · eshopeo.cloudia.co.za · You're receiving this because you signed up for eShopeo.
      </p>
    </div>"""
...
@celery_app.task(name="eshopeo.notifications.email.send_welcome_email")
def send_welcome_email(
    ...
    html = _base(f"""
      <h2 style="margin:0 0 8px;">Welcome to eShopeo, {store_name}! 🎉</h2>
      ...
      <a href="https://eshopeo.cloudia.co.za/v1/plugin/download"
    ...
    """)
    _send(_api_key(), email, "Welcome to eShopeo — your trial is active", html)


@celery_app.task(name="eshopeo.notifications.email.send_magic_link_email")
def send_magic_link_email(email: str, store_name: str, magic_link: str) -> None:
    html = _base(f"""
      <h2 style="margin:0 0 8px;">Your eShopeo login link</h2>
      <p>Hi {store_name}, click the button below to sign in to your eShopeo dashboard.
    ...
    """)
    _send(_api_key(), email, "Your eShopeo login link", html)


@celery_app.task(name="eshopeo.notifications.email.send_trial_reminder")
def send_trial_reminder(email: str, store_name: str, days_remaining: int) -> None:
    urgency = "🔔" if days_remaining > 3 else "⚠️"
    html = _base(f"""
      <h2 style="margin:0 0 8px;">{urgency} {days_remaining} day{'s' if days_remaining != 1 else ''} left in your eShopeo trial</h2>
      ...
      <a href="https://eshopeo.cloudia.co.za/billing"
         style="display:inline-block;background:#7c3aed;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:12px 0;">
        Choose a Plan
      </a>
      <p style="color:#6b7280;font-size:13px;">Plans start at R699/mo (or $39/mo). Cancel any time.</p>
    """)
    _send(_api_key(), email, f"Your eShopeo trial ends in {days_remaining} day{'s' if days_remaining != 1 else ''}", html)


@celery_app.task(name="eshopeo.notifications.email.send_trial_expired_email")
def send_trial_expired_email(email: str, store_name: str) -> None:
    html = _base(f"""
      <h2 style="margin:0 0 8px;">Your eShopeo trial has ended</h2>
    ...
    """)
    _send(_api_key(), email, "Your eShopeo trial has ended — reactivate now", html)


@celery_app.task(name="eshopeo.notifications.email.send_subscription_confirmed")
def send_subscription_confirmed(email: str, store_name: str, tier: str) -> None:
    plan_name = tier.title()
    html = _base(f"""
      <h2 style="margin:0 0 8px;">You're on eShopeo {plan_name} ✅</h2>
      ...
      <a href="https://eshopeo.cloudia.co.za/billing/portal"
         style="display:inline-block;background:#7c3aed;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:12px 0;">
        Manage Subscription
      </a>
    """)
    _send(_api_key(), email, f"You're subscribed to eShopeo {plan_name}", html)


@celery_app.task(name="eshopeo.notifications.email.send_payment_failed_email")
def send_payment_failed_email(email: str, store_name: str) -> None:
    html = _base(f"""
      <h2 style="margin:0 0 8px;">⚠️ Payment failed for {store_name}</h2>
      <p>We couldn't process your last eShopeo payment. Your account has been paused.</p>
    ...
    """)
    _send(_api_key(), email, "Action needed: eShopeo payment failed", html)


@celery_app.task(name="eshopeo.notifications.email.send_cancellation_email")
def send_cancellation_email(email: str, store_name: str) -> None:
    html = _base(f"""
      <h2 style="margin:0 0 8px;">Your eShopeo subscription has been cancelled</h2>
    ...
    """)
    _send(_api_key(), email, "Your eShopeo subscription has been cancelled", html)
