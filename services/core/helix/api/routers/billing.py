...
from helix.billing.paddle import PaddleClient, parse_event, verify_webhook
from helix.billing.plans import PLANS, get_plan, get_ai_ops_limit, get_query_limit, plan_from_paddle_price
...
async def _on_subscription_created(tenant: Tenant, data: dict, db: AsyncSession) -> None:
    ...
    try:
        from helix.notifications.email import send_subscription_confirmed
...
async def _on_subscription_cancelled(tenant: Tenant, data: dict, db: AsyncSession) -> None:
    ...
    try:
        from helix.notifications.email import send_cancellation_email
...
async def _on_payment_failed(tenant: Tenant, data: dict, db: AsyncSession) -> None:
    ...
    try:
        from helix.notifications.email import send_payment_failed_email
