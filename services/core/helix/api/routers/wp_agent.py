"""
Helix WP Agent router — Claude tool_use AI assistant for WordPress site management.
...
_SYSTEM_PROMPT = """You are the Helix WP Agent — an expert WordPress site manager embedded
in the Helix AI admin panel.  You have access to a set of tools that let you
...
WP_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Run a Helix quick action on the WordPress site. "
    ...
    {
        "name": "get_action_log",
        "description": "Retrieve the recent Helix action history (quick actions that have been run, including dry-runs).",
        "input_schema": {
            "type": "object",
            "properties": {
                "limit": {
...
async def _tool_get_performance_metrics(site_url: str, admin_secret: str) -> Any:
    ...
    url = site_url.rstrip("/") + "/wp-admin/admin-ajax.php"
    data = {
        "action": "helix_performance_metrics",
        "helix_agent_secret": admin_secret,
    }
...
async def _tool_get_action_log(
    ...
    """Fetch the Helix action log from the WordPress site."""
    limit: int = min(int(inp.get("limit", 20)), 100)
    url = site_url.rstrip("/") + "/wp-admin/admin-ajax.php"
    data = {
        "action": "helix_get_action_log",
        "limit": str(limit),
        "helix_agent_secret": admin_secret,
    }
